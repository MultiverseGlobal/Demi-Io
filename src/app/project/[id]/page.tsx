...
import { validateIntent, sanitizeManifest, validateGuardrails, TrustReport, SafetyIssue } from "@/lib/intelligence";
...
function ProjectEditorContent({ params }: { params: { id: string } }) {
    ...
    const [guardrailIssues, setGuardrailIssues] = useState<SafetyIssue[]>([]);
    ...
    const [showPublishingSuite, setShowPublishingSuite] = useState(false);
    const { is_pro, available_credits, loading } = useSubscription();

    const handleDownloadZip = () => {
        setShowPublishingSuite(true);
    };

    const handleFinalDownload = async (additionalFiles: Record<string, string> = {}) => {
        const zip = new JSZip();

        // Add all files to the ZIP
        Object.entries({ ...files, ...additionalFiles }).forEach(([filename, content]) => {
    ...
    // Subscription Guard
    const isPremiumModel = ['gpt-4o', 'gemini-1.5-pro'].includes(selectedModel);
    if (isPremiumModel && !is_pro) {
        alert("This model is only available for Pro users. Please upgrade to use advanced reasoning.");
        return;
    }

    if (available_credits < 50) {
        alert("Insufficient credits. Please upgrade or top up your balance.");
        return;
    }
    ...
    const { error: creditError } = await supabase
        .from('profiles')
        .update({ available_credits: available_credits - 50 })
        .eq('id', user.id);
...
return (
    <div className="flex h-screen w-full bg-white text-neutral-900 overflow-hidden font-sans">
        ...
        <div className="flex-1 flex overflow-hidden relative">
            ...
            <div id="agent-chat-panel" className="w-[380px] flex-shrink-0 flex flex-col border-r border-neutral-200 bg-neutral-50/50 backdrop-blur-xl">
                {/* Header */}
                ...
                <div className="p-4 border-t border-neutral-200 bg-white">
                    ...
                    <div className="relative group bg-neutral-50 border border-neutral-200 focus-within:border-blue-500/40 rounded-xl transition-all duration-200 shadow-sm">
                        ...
                        <div className="flex items-center gap-4 px-4 py-2 border-t border-neutral-100 bg-white/50">
                            ...
                            <div className="w-1 h-1 rounded-full bg-neutral-200" />
                            <div id="selector-intelligence-btn" className={cn(
                                "flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-wider transition-colors",
                                guardrailIssues.some(i => i.severity === 'critical') ? "text-red-500 animate-pulse" : guardrailIssues.length > 0 ? "text-yellow-500" : "text-blue-600"
                            )}>
                                <Shield className="w-3 h-3" />
                                {guardrailIssues.length > 0 ? `${guardrailIssues.length} Safety Flag${guardrailIssues.length > 1 ? 's' : ''}` : "Guardrails Passed"}
                            </div>
                        </div>
                        ...
                        <div className="px-3 pb-3 flex items-center justify-between">
                            ...
                            <div className="relative">
                                ...
                                >
                                    ...
                                    <option value="gpt-4o">GPT-4o (OpenAI) {!is_pro ? " [PRO]" : ""}</option>
                                    <option value="gemini-1.5-pro">Gemini 1.5 Pro {!is_pro ? " [PRO]" : ""}</option>
                                </select>
                ...
                <div className="flex-1 relative overflow-hidden bg-neutral-100">
                    {activeTab === 'preview' ? (
                        ...
                        <div className="flex h-full">
                            ...
                            {/* Code Editor View */}
                            <div id="code-editor-section" className="flex-1 p-8 overflow-auto font-mono text-sm text-neutral-600 bg-white">
        ...
        />
        <PublishingSuite
            isOpen={showPublishingSuite}
            onClose={() => setShowPublishingSuite(false)}
            files={files}
            guardrailIssues={guardrailIssues}
            projectName={project?.name || "Demi Extension"}
                                            ...
                                            {guardrailIssues.map((issue, i) => (
                                                ...
                                                >
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        {issue.file && (
                                                            <span className="text-[9px] font-mono text-neutral-500">{issue.file}</span>
                                                        )}
                                                    </div>
                                                    <h4 className="text-sm font-bold text-white">{issue.title}</h4>
                                                    <p className="text-xs text-neutral-400 leading-relaxed">{issue.message}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </section>
